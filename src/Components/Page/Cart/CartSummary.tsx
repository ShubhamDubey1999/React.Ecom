...
function CartSummary() {
  ...
  if (!shoppingCartFromStore) {
    return <div className="text-center">Shopping Cart Empty</div>;
  }
  const handleQuantity = (
    updateQunatityBy: number,
    cartItem: cartItemModel
  ) => {
    if (
      (updateQunatityBy == -1 && cartItem.quantity == 1) ||
      updateQunatityBy == 0
    ) {
      updateShoppingCart({
      ...
      updateShoppingCart({
        menuItemId: cartItem.menuItem?.id,
        updateQuantityBy: updateQunatityBy,
        userId: userData.id,
      });
    }
    dispatch(
      updateQuantity({
        cartItem,
        quantity: cartItem.quantity! + updateQunatityBy,
      })
    );
  };
  return (
